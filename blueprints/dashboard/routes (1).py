...
def index():
    ...
    if is_admin:
        ...
        if user:
            ...
            # Add account diagnostics and user status
            stats['is_verified'] = user.is_verified
            stats['is_demo'] = user.is_demo
            stats['account_diagnostics'] = None
            if user.is_demo or user.is_verified:
                stats['account_diagnostics'] = calculate_account_diagnostics(data, username, user.is_demo, user.is_verified)
        else:
            # Fallback
            stats = {'projects': 0, 'skills': 0, 'clients': 0, 'services': 0, 'messages': 0, 'unread_messages': 0, 'visitors': 0, 'today_visitors': 0, 'is_demo': True, 'account_diagnostics': None}
